...
export async function getLeads({ page = 1, perPage = 10, search = "" }: LeadsParams = {}): Promise<LeadsResponse> {
  try {
    await requireAdmin()
    ...
    downloads.forEach((download) => {
      ...
      if (existing) {
        ...
        existing.firstDownload = new Date(Math.min(existing.firstDownload.getTime(), download.createdAt.getTime()))
        existing.lastDownload = new Date(Math.max(existing.lastDownload.getTime(), download.createdAt.getTime()))
        ...
        emailMap.set(email, {
          ...
          resources: new Set(download.resource?.title ? [download.resource.title] : []),
        })
      }
    })
...
export async function exportLeadsToCSV(): Promise<{
  ...
  try {
    ...
    // Create CSV headers
    const headers = ["Email", "Download Count", "First Download", "Last Download", "Resources Downloaded"]
...
export async function getLeadsStats() {
  try {
    ...
    const avgDownloadsPerLead = totalLeads > 0 ? totalDownloads / totalLeads : 0
