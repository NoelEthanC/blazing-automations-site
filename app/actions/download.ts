...
import { generateDownloadToken, validateEmail } from "@/lib/download-utils"
...
export async function downloadResourceAction(formData: FormData): Promise<DownloadResult> {
  try {
    ...
    const downloadToken = generateDownloadToken(resource.id, email)
    ...
    if (action === "send_email") {
      const emailHtml = generateDownloadEmailTemplate(resource.title, downloadUrl)
