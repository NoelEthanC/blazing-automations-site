...
import { FileText, Download, MessageSquare, TrendingUp, Eye } from "lucide-react"
import { prisma } from "@/lib/prisma"

async function getDashboardStats() {
  try {
    const [totalResources, publishedResources, totalDownloads, totalContacts, recentDownloads] = await Promise.all([
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-400">Welcome back! Here's what's happening with your automation resources.</p>
      </div>
              ...
              {stats.recentDownloads.length > 0 ? (
                stats.recentDownloads.map((download) => (
                  <div key={download.id} className="flex items-center justify-between">
                    <div>
                      <p className="text-white font-medium">{download.resource.title}</p>
                      <p className="text-sm text-gray-400">{download.email}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm text-gray-400">{new Date(download.createdAt).toLocaleDateString()}</p>
                ...
                <p className="text-gray-400 text-center py-4">No downloads yet</p>
              )}
            </div>
          </CardContent>
        </Card>
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400">Create a new automation template</p>
                </div>
              </a>
