import { getAllResources, toggleResourceStatus, deleteResource } from "@/app/actions/resources"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { Badge } from "@/components/ui/badge"
import { Button } from "@/components/ui/button"
import { Plus, Edit, Trash2, Eye, EyeOff, Star, StarOff, Download, Calendar } from "lucide-react"
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400">Manage your automation templates and guides.</p>
        </div>
            ...
            <Card key={resource.id} className="bg-gray-800/50 border-gray-700">
              ...
              <CardContent>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <form action={toggleResourceStatus.bind(null, resource.id, "published")}>
                    <Button
                      type="submit"
                      size="sm"
                      variant="ghost"
                      className={resource.published ? "text-green-400" : "text-gray-400"}
                    >
                      {resource.published ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
                    </Button>
                  </form>

                  <form action={toggleResourceStatus.bind(null, resource.id, "featured")}>
                    <Button
                      type="submit"
                      size="sm"
                      variant="ghost"
                      className={resource.featured ? "text-yellow-400" : "text-gray-400"}
                    >
                      {resource.featured ? <Star className="h-4 w-4" /> : <StarOff className="h-4 w-4" />}
                    </Button>
                  </form>
