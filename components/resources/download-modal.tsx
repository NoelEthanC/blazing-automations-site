...
import { confirmLead, validateEmail } from "@/lib/download-utils";
...
export function DownloadModal({
  ...
  useEffect(() => {
    ...
    const verifyToken = async () => {
      ...
      try {
        ...
        if (res.ok) {
          // Fetch the JSON from Supabase
          const fileRes = await fetch(data.downloadUrl);
          const text = await fileRes.text(); // for JSON
          const blob = new Blob([text], { type: "application/json" });
          // auto-start download
          const link = document.createElement("a");
          link.href = URL.createObjectURL(blob);
          link.download = data.filename || "download.json";
          ...
          // TODO: confirm lead in the background
          await fetch("/api/confirm-download", {
            method: "POST",
          ...
          setError(error || "Invalid or expired link");
          downloadTriggeredRef.current = false; // reset on failure
        }
      } catch (error) {
        setError("Invalid or expired link. Try again!");
        downloadTriggeredRef.current = false; // reset on failure
      } finally {
    ...
    verifyToken();
  }, [showSuccess, downloadToken, resourceSlug, onClose]);
  ...
  const handleDownload = async (action: "download_now" | "send_email") => {
    ...
    trackEvent(
      ...Object.values(events.resources.download(resourceSlug, action))
    );
        ...
        <DialogContent className="sm:max-w-md bg-gray-900 border-gray-800 text-white">
          {isVerifying ? (
            <div className="flex flex-col items-center py-8 space-y-4">
              ...
              <p className="text-white">Your template is loading...</p>
            </div>
          ) : error ? (
            <div className="flex flex-col items-center py-8 space-y-4">
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-md bg-gray-900 border-gray-800 text-white">
        <DialogHeader>
          <DialogTitle className="text-3xl font-semibold gradient-text">
        ...
        {showSuccess ? (
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sunray text-sm font-light">
                Enter your email address to download this template.
              </p>
            </div>
            {/* form inputs */}
            <>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <Button
                ...
                disabled={
                  !email.trim() || !validateEmail(email.trim()) || isSubmitting
                }
            ...
            <p className="text-xs text-gray-500 text-center">
              By downloading, you agree to receive occasional updates about our
